...
export default function WinnersPage() {
  ...
  return (
    <AppShell>
      ...
      {isLoading ? (
        <div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{[0, 1, 2, 3].map((item) => <Card key={item} className="h-[330px] animate-pulse bg-[#0f141d]" />)}</div>
      ) : errorMessage ? (
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{visibleWinners.map((winner) => <div key={winner.id} className="min-w-0 space-y-3"><WinnerCard submission={winner} /><div className="rounded-[8px] border border-white/10 bg-black/30 p-3 text-sm leading-6 text-slate-300"><b className="text-[var(--gold)]">#{winner.position ?? "-"}</b> <span className="break-words">- {String(winner.winnerStatus ?? "announced").replaceAll("_", " ")}{winner.payoutStatus && winner.payoutStatus !== "not_applicable" ? ` - Payout review: not active yet (${String(winner.payoutStatus).replaceAll("_", " ")})` : ""}</span></div></div>)}</div>
      ) : (
