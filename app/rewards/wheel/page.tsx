...
export default function RewardWheelPage() {
  ...
  return <AppShell>
    ...
    <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <div className="space-y-6"><Card className="p-5"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Reward Points</p><p className="mt-2 text-3xl font-black text-[var(--gold)]">{data ? points.toLocaleString() : "..."}</p><p className="mt-2 text-sm text-slate-400">This tier costs {cost.toLocaleString()} points.</p></Card><Card className="p-5"><h2 className="text-xl font-black">Possible Rewards</h2><p className="mt-2 text-sm text-slate-400">These rewards come from the same published pool shown on the wheel.</p><div className="mt-4 space-y-3">{!data ? Array.from({ length: 5 }, (_, index) => <div key={index} className="h-9 animate-pulse rounded-[6px] bg-[var(--panel-2)]" />) : slices.length ? slices.map((slice) => <div key={slice.id} className="border-b border-white/10 pb-3 text-sm"><span className="font-bold">{slice.prizeName}</span></div>) : <p className="rounded-[8px] border border-[var(--line)] bg-[var(--panel-2)] p-4 text-sm text-[var(--muted)]">No eligible rewards are available for this tier right now.</p>}</div></Card></div>
    </div>
