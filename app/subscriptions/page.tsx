...
export default function SubscriptionsPage() {
  ...
  return <AppShell><div className="text-center"><PageTitle title="Choose Your Plan" subtitle="Upgrade normal Challenge Suite access for competitors, creators, and hosts." /></div>
    <Card className="mx-auto mt-6 max-w-4xl border-yellow-500/20 bg-yellow-500/5 p-4 text-center text-sm leading-6 text-slate-300">Creator and Host plans unlock platform tools. Paid-entry prize pools and automatic cash payouts remain disabled.</Card>
    ...
    {loading ? <div className="mt-14 grid gap-7 md:grid-cols-3">{[1, 2, 3].map((item) => <Card key={item} className="h-[520px] animate-pulse bg-[#10151e]" />)}</div> : unauthenticated ? <Card className="mx-auto mt-14 max-w-3xl"><EmptyState icon={<LockKeyhole />} title="Sign in required" body={error || "Sign in with a verified account to view your current subscription options."} action={<LinkButton href="/auth/login">Sign In</LinkButton>} /></Card> : !visiblePlans.length ? <Card className="mx-auto mt-14 max-w-3xl"><EmptyState icon={<Swords />} title="No plans available" body="Subscription plans are not available yet." action={<Button onClick={loadPlans}>Retry</Button>} /></Card> : <div className="mx-auto mt-14 grid max-w-[1180px] gap-7 md:grid-cols-3">{visiblePlans.map((plan) => <Card key={plan.id} className={`relative p-8 ${plan.recommended ? "border-[var(--gold)] bg-yellow-500/10" : "bg-[#10151e]"}`}>{plan.recommended ? <span className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-[var(--gold)] px-7 py-2 text-sm font-bold text-black">Recommended</span> : null}<div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[var(--gold)]/10 text-[var(--gold)]">{plan.id === "free" ? <Eye size={34} /> : plan.id === "host" ? <Radio size={34} /> : <Swords size={34} />}</div><h2 className="mt-8 text-center text-3xl font-black">{plan.name}</h2><p className="mt-4 text-center text-slate-300">{plan.subtitle}</p><div className="mt-6 text-center text-4xl font-black">{plan.priceMonthlyLabel ?? (plan.priceMonthly !== null ? money(plan.priceMonthly) : "Pricing pending")}</div>{plan.id === "free" ? <p className="mt-3 rounded-[8px] bg-black/30 p-3 text-center text-xs font-bold text-slate-300">1 free vote per challenge/day.</p> : null}<div className="my-8 border-t border-white/10" /><ul className="space-y-4">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm"><Check size={16} className="shrink-0 text-emerald-400" /> {feature}</li>)}</ul><div className="mt-8">{signedOut && plan.id !== "free" ? <LinkButton href="/auth/login" className="w-full">Sign In to Choose Plan</LinkButton> : plan.current ? <Button variant="ghost" className="w-full" disabled>Current Plan</Button> : plan.id === "free" ? <Button variant="ghost" className="w-full" disabled>Free Plan</Button> : !plan.purchaseAllowed ? <Button variant="ghost" className="w-full" disabled>Not Available for This Account</Button> : !plan.checkoutAvailable ? <Button variant="ghost" className="w-full" disabled>Stripe Price Not Configured</Button> : <ConsentDialog agreementType="dorocoin" actionLabel={loadingPlan === plan.id ? "Starting Checkout..." : "Select Plan"} onAccepted={() => checkout(plan.id)} />}</div></Card>)}</div>}
    <Card className="mx-auto mt-10 max-w-4xl border-[var(--gold)]/25 bg-[var(--gold)]/5 p-6 text-center"><h2 className="text-2xl font-black">Need Enterprise access?</h2><p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-300">Enterprise access is available by application for approved teams and in-house operators.</p><LinkButton href="/enterprise/apply" className="mt-5">Apply for Enterprise Access</LinkButton></Card>
  </AppShell>;
}
