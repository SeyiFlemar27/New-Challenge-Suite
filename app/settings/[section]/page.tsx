...
function Billing({ settings, confirmCancel, setConfirmCancel, canceling, cancelSubscription }: { settings: SettingsData; confirmCancel: boolean; setConfirmCancel: (value: boolean) => void; canceling: boolean; cancelSubscription: () => Promise<void> }) {
  ...
  return <div><div className="flex items-center gap-3"><CreditCard className="text-[var(--gold)]" /><h2 className="text-xl font-black">Current Subscription</h2></div><div className="mt-6 rounded-[8px] bg-black/30 p-5"><p className="font-black">{settings.account.effectiveTier?.displayName ?? settings.account.planId}</p><p className="mt-1 text-sm capitalize text-slate-400">Status: {settings.account.subscriptionStatus.replaceAll("_", " ")} Â· Billing cycle appears after Stripe invoice sync.</p></div><div className="mt-5 flex flex-wrap gap-3"><LinkButton href="/subscriptions">{upgradeLabel}</LinkButton><Button variant="secondary" disabled>View Invoices (After Stripe Sync)</Button><Button variant="secondary" disabled>Payment History (Not Available Yet)</Button>{hasPaidPlan ? <Button variant="secondary" onClick={() => setConfirmCancel(true)}>Cancel Subscription</Button> : null}</div>{confirmCancel ? <div className="mt-6 rounded-[8px] border border-red-500/30 bg-red-950/20 p-5"><h3 className="font-black text-red-100">Confirm cancellation</h3><p className="mt-2 text-sm leading-6 text-red-100/80">Are you sure you want to cancel your subscription? Cancelling immediately returns your account to Free access after the verified Stripe webhook is received.</p><div className="mt-4 flex flex-wrap gap-3"><Button variant="secondary" onClick={() => setConfirmCancel(false)}>Keep Subscription</Button><Button onClick={() => void cancelSubscription()} disabled={canceling}>{canceling ? "Requesting..." : "Confirm Cancellation"}</Button></div></div> : null}</div>;
}
