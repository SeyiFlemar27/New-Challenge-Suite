...
const emptyForm = {
  ...
  logoUrl: "", alternateLogoUrl: "", squareIconUrl: "", bannerUrl: "", coverImageUrl: "", brandColorsText: "#F5B700", brandFontsText: "", brandTone: "", socialLinksText: "", approvedHashtagsText: "", preferredCtaLabels: ["Learn More"] as string[], ctaButtonText: "Learn More", ctaDestinationLink: "",
...
export default function SponsorOnboardingPage() {
  ...
  const payload = useMemo(() => ({
    ...
    logoUrl: form.logoUrl, alternateLogoUrl: form.alternateLogoUrl, squareIconUrl: form.squareIconUrl, bannerUrl: form.bannerUrl, coverImageUrl: form.coverImageUrl, brandColors: lines(form.brandColorsText), brandFonts: lines(form.brandFontsText), brandTone: form.brandTone, socialLinks: lines(form.socialLinksText), approvedHashtags: lines(form.approvedHashtagsText), preferredCtaLabels: form.preferredCtaLabels, ctaButtonText: form.ctaButtonText, ctaDestinationLink: form.ctaDestinationLink,
  ...
  return <SponsorShell profile={profile}>
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="text-sm font-black uppercase tracking-[0.22em] text-[var(--gold)]">Sponsor Onboarding</p><h1 className="mt-3 text-3xl font-black leading-tight sm:text-4xl lg:text-5xl">{completed ? "Update your brand command center" : "Build your brand command center"}</h1><p className="mt-4 max-w-3xl text-base leading-7 text-slate-300 sm:text-lg">Complete business, identity, audience, budget, and verification foundations. Sponsors can explore the dashboard while review is pending, but restricted money and high-value tools stay locked.</p></div>
      ...
      <form className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,1fr)_360px]" onSubmit={saveProfile}>
        <Card className="p-5 sm:p-6 lg:p-8">
          ...
          {step === 1 ? <><Grid><MediaUploadField label="Primary Logo" value={form.logoUrl} onChange={(url) => update("logoUrl", url)} storagePath={`sponsorMedia/profile/${auth.user?.uid ?? "anonymous"}/logo`} kind="image" buttonLabel="Upload Logo" /><MediaUploadField label="Alternate Logo" value={form.alternateLogoUrl} onChange={(url) => update("alternateLogoUrl", url)} storagePath={`sponsorMedia/profile/${auth.user?.uid ?? "anonymous"}/alternate-logo`} kind="image" buttonLabel="Upload Alternate Logo" /><MediaUploadField label="Square Brand Icon" value={form.squareIconUrl} onChange={(url) => update("squareIconUrl", url)} storagePath={`sponsorMedia/profile/${auth.user?.uid ?? "anonymous"}/square-icon`} kind="image" buttonLabel="Upload Icon" /><MediaUploadField label="Cover Image" value={form.bannerUrl} onChange={(url) => { update("bannerUrl", url); update("coverImageUrl", url); }} storagePath={`sponsorMedia/profile/${auth.user?.uid ?? "anonymous"}/cover`} kind="image" buttonLabel="Upload Cover" /></Grid><Grid><Area field="brandColorsText" label="Brand Colors" form={form} update={update} /><Area field="brandFontsText" label="Brand Fonts" form={form} update={update} /><Select field="brandTone" label="Brand Tone" options={sponsorToneOptions} form={form} update={update} /><Area field="socialLinksText" label="Social Media Links" form={form} update={update} /><Area field="approvedHashtagsText" label="Approved Hashtags" form={form} update={update} /><div><p className="mb-2.5 text-sm font-bold text-white">Preferred CTA Labels</p><div className="flex flex-wrap gap-2">{sponsorCtaLabels.map((label) => <Chip key={label} label={label} active={form.preferredCtaLabels.includes(label)} onClick={() => toggleList("preferredCtaLabels", label)} />)}</div></div></Grid><Field label="Company Description"><textarea className={textareaClass} value={form.brandDescription} onChange={(event) => update("brandDescription", event.target.value)} />{fieldErrors.brandDescription ? <ErrorText>{fieldErrors.brandDescription}</ErrorText> : null}</Field></> : null}
          ...
          {step === 5 ? <div className="space-y-5"><Card className="border-yellow-500/20 bg-yellow-500/5 p-5"><ShieldCheck className="text-[var(--gold)]" /><h3 className="mt-3 text-xl font-black">Business verification foundation</h3><p className="mt-2 text-sm leading-6 text-slate-300">Verification status is reviewed by Challenge Suite admins. Sponsors can explore the dashboard while pending, but funding campaigns, verified badge, high-value sponsor tools, payment release actions, and enterprise sponsorship tools stay restricted.</p></Card><div className="grid gap-3 sm:grid-cols-2">{sponsorVerificationDocuments.map((doc) => <div key={doc} className="rounded-[8px] border border-white/10 bg-[#171717] p-4 text-sm font-bold text-slate-300">{doc}<p className="mt-2 text-xs font-normal text-slate-500">Metadata/secure references only. Raw documents are not stored in app records.</p></div>)}</div></div> : null}
