...
export default function SponsorCreatorDiscoveryPage() {
  ...
  const urlPage = Math.max(1, Math.floor(Number(searchParams.get("page") ?? 1) || 1));
  ...
  const [pagination, setPagination] = useState<Pagination>({ page: 1, pageSize: 36, total: 0, totalPages: 1 });
  ...
  useEffect(() => { setQuery(urlQuery); void load(urlQuery, urlPage); }, [urlPage, urlQuery]);
  async function load(search = urlQuery, page = urlPage) {
    setLoading(true);
    const [profileResult, creatorResult, savedResult, invitationResult] = await Promise.all([
      ...
      apiRequest<{ creators: Creator[]; pagination: Pagination }>(`/api/sponsor/discover/creators?q=${encodeURIComponent(search)}&page=${page}`),
    ...
    if (creatorResult.ok && creatorResult.data) { setCreators(creatorResult.data.creators); setPagination(creatorResult.data.pagination); setError(""); } else setError(creatorResult.message || "Creators could not be loaded.");
  ...
  function navigate(search: string, page: number) {
  ...
  return <SponsorShell profile={profile}><div className="mx-auto max-w-7xl"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm font-black uppercase tracking-[0.2em] text-[var(--gold)]">Creator Discovery</p><h1 className="mt-3 text-4xl font-black">Find sponsor-ready creators</h1><p className="mt-3 max-w-3xl leading-7 text-slate-600">Only real backend creators with completed profiles and sponsor-ready status appear here. Missing performance metrics are shown as Not available yet.</p></div><div className="flex flex-wrap gap-3"><LinkButton href={compare.length ? `/sponsor/discover/creators/compare?ids=${compare.join(",")}` : "/sponsor/discover/creators/compare"} variant="secondary">Compare ({compare.length}/4)</LinkButton><Button variant="ghost" onClick={() => setCompare([])}>Clear</Button></div></div><Card className="mt-8 p-4"><form className="flex flex-col gap-3 lg:flex-row" onSubmit={submitSearch}><div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-3.5 text-slate-500" size={18} /><input className={`${inputClass} pl-10`} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by niche, category, country, city, or verified status" /></div><Button type="submit">Search</Button><div className="grid grid-cols-2 gap-2 sm:flex">{(["discover", "saved", "pending", "active"] as Tab[]).map((item) => <Button key={item} type="button" variant={tab === item ? "primary" : "secondary"} onClick={() => setTab(item)}>{item[0].toUpperCase() + item.slice(1)}</Button>)}</div></form></Card>{notice ? <Card className="mt-5 p-4 text-sm text-slate-600">{notice}</Card> : null}{error ? <Card className="mt-6 border-red-500/20 border border-red-200 bg-red-50 p-5 text-red-800">{error}</Card> : null}{loading ? <div className="mt-8 grid gap-4">{[0,1,2].map((item) => <Card key={item} className="h-40 animate-pulse bg-slate-100" />)}</div> : visibleCreators.length === 0 ? <EmptyState icon={<Users />} title={tab === "discover" ? "No sponsor-ready creators yet." : `No ${tab} creators yet.`} body={tab === "discover" ? "Creators will appear after a real creator profile is completed and marked sponsor-ready." : "This section updates from saved creators, pending invitations, and active sponsor relationships."} /> : <><div className="sponsor-discovery-list mt-8 grid gap-5">{visibleCreators.map((creator) => <CreatorCard key={creator.id} creator={creator} saved={savedIds.has(String(creator.id))} pending={pendingIds.has(String(creator.id))} active={activeIds.has(String(creator.id))} compared={compare.includes(creator.id)} onSave={() => void saveCreator(creator.id)} onCompare={() => toggleCompare(creator.id)} onInvite={() => { setInviteCreator(creator); setInviteMessage(`I would like to discuss a ${inviteType === "custom_challenge_request" ? "custom challenge" : "sponsor campaign"} opportunity with you.`); }} />)}</div><PaginationControls pagination={pagination} onPage={(page) => navigate(urlQuery, page)} /></>}{inviteCreator ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><Card className="w-full max-w-xl p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--gold)]">Invite Creator</p><h2 className="mt-2 text-2xl font-black">{inviteCreator.displayName}</h2></div><Button variant="ghost" onClick={() => setInviteCreator(null)}><X size={16} /></Button></div><div className="mt-5 grid gap-4"><Field label="Invite type"><select className={inputClass} value={inviteType} onChange={(event) => setInviteType(event.target.value)}><option value="custom_challenge_request">Custom Challenge Request</option><option value="join_sponsor_campaign">Join Sponsor Campaign</option></select></Field><Field label="Message"><textarea className={textareaClass} value={inviteMessage} onChange={(event) => setInviteMessage(event.target.value)} /></Field><Button disabled={inviteMessage.trim().length < 2} onClick={() => void sendInvite()}><Send size={16} /> Send Pending Invite</Button><p className="text-xs leading-5 text-slate-500">The invite remains pending until the creator/host responds. No acceptance, email delivery, or sponsorship agreement is created.</p></div></Card></div> : null}</div></SponsorShell>;
}

function PaginationControls({ pagination, onPage }: { pagination: Pagination; onPage: (page: number) => void }) {
  if (pagination.totalPages <= 1) return null;
  return <nav className="mt-7 flex items-center justify-between gap-4" aria-label="Creator discovery pages"><Button variant="secondary" disabled={pagination.page <= 1} onClick={() => onPage(pagination.page - 1)}>Previous</Button><p className="text-sm font-bold text-slate-600">Page {pagination.page} of {pagination.totalPages}</p><Button variant="secondary" disabled={pagination.page >= pagination.totalPages} onClick={() => onPage(pagination.page + 1)}>Next</Button></nav>;
}
