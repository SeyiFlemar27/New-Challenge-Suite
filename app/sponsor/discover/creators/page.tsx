...
function CreatorCard({ creator, saved, pending, active, compared, onSave, onCompare, onInvite }: { creator: Creator; saved: boolean; pending: boolean; active: boolean; compared: boolean; onSave: () => void; onCompare: () => void; onInvite: () => void }) {
  return <Card className="p-5 sm:p-6"><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(520px,1fr)] xl:items-center"><div className="flex min-w-0 items-start gap-4"><div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full border border-[var(--gold)]/40 bg-black text-xl font-black text-[var(--gold)]">{creator.avatarUrl ? <img src={creator.avatarUrl} alt="" className="h-full w-full object-cover" /> : String(creator.displayName ?? "C").slice(0,1)}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="break-words text-xl font-black">{creator.displayName}</h2>{active ? <Chip label="Active" /> : pending ? <Chip label="Pending" /> : saved ? <Chip label="Saved" /> : <Chip label="Discover" />}</div><p className="text-sm text-slate-400">@{creator.username}</p><p className="mt-1 text-xs font-black uppercase tracking-[0.14em] text-[var(--gold)]">{creator.verificationStatus || "Not available yet"}</p></div></div><div><div className="grid gap-3 text-sm text-slate-300 sm:grid-cols-3"><Info label="Niche" value={creator.niche} /><Info label="Audience" value={creator.audienceSizeLabel} /><Info label="Engagement" value={creator.engagementRateLabel} /></div><div className="mt-5 flex flex-wrap gap-3 xl:justify-end"><Button variant={saved ? "primary" : "ghost"} onClick={onSave}><Bookmark size={16} /> {saved ? "Saved" : "Save"}</Button><LinkButton href={`/sponsor/discover/creators/${creator.id}`} variant="secondary">View Profile</LinkButton><Button variant={compared ? "primary" : "ghost"} onClick={onCompare}>Compare</Button><Button onClick={onInvite}><Send size={16} /> Invite</Button></div></div></div></Card>;
}
...
function Info({ label, value }: { label: string; value: any }) { return <div className="rounded-[8px] bg-black/30 p-3"><p className="text-[11px] font-black uppercase tracking-[0.14em] text-slate-500">{label}</p><p className="mt-1 font-bold text-white">{value || "Not available yet"}</p></div>; }
