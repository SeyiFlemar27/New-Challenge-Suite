...
export default function SponsorSettingsPage() {
  ...
  return <SponsorShell profile={profile}>{loading ? <Card className="h-80 animate-pulse bg-[#171717]" /> : error ? <Card className="border-red-500/20 bg-red-950/30 p-6 text-red-200">{error}</Card> : <div className="mx-auto max-w-6xl"><p className="text-sm font-black uppercase tracking-[0.22em] text-[var(--gold)]">Sponsor Settings</p><h1 className="mt-3 text-4xl font-black">Account and brand controls</h1><p className="mt-3 max-w-3xl text-slate-300">Settings are organized around brand profile, verification, team, billing, notifications, security, integrations, privacy, and account controls. Destructive actions require confirmation and server validation.</p><div className="mt-8 grid gap-5 lg:grid-cols-2">{sections.map((section) => <Card key={section.title} className="p-6"><section.icon className="text-[var(--gold)]" /><h2 className="mt-3 text-xl font-black">{section.title}</h2><p className="mt-2 text-sm leading-6 text-slate-300">{section.body}</p>{section.href ? <LinkButton href={section.href} variant="secondary" className="mt-5">Open</LinkButton> : null}</Card>)}</div><Card className="mt-8 border-yellow-500/20 bg-yellow-500/5 p-6"><ShieldCheck className="text-[var(--gold)]" /><h2 className="mt-3 text-xl font-black">Current status</h2><p className="mt-2 text-sm text-slate-300">Plan: {profile?.planId?.replaceAll("_", " ") || "No sponsor plan"} / Subscription: {subStatus.replaceAll("_", " ")} / Verification: {businessVerificationLabel(verification)}</p></Card><Card className="mt-8 border-red-500/20 bg-red-950/20 p-6"><AlertTriangle className="text-red-200" /><h2 className="mt-3 text-xl font-black">Destructive actions require confirmation</h2><p className="mt-2 text-sm leading-6 text-slate-300">Cancellation, account deletion, and destructive brand changes remain confirmation-gated. No sponsor wallet release, refund, prize release, or payout action is available here.</p></Card></div>}</SponsorShell>;
}
const sections = [
  ...
  { title: "Business information", body: "Legal business details and operating profile are managed through onboarding and profile editing.", icon: Settings, href: "/sponsor/onboarding" },
  ...
  { title: "Team", body: "Team seats, invites, role permissions, and remove/transfer controls.", icon: Users, href: "/sponsor/team" },
  { title: "Billing and subscription", body: "Current plan, subscription status, renewal, upgrade/downgrade, cancellation, and invoice history.", icon: CreditCard, href: "/sponsor/billing" },
  ...
  { title: "Security", body: "Security review, access controls, and audit history for sensitive sponsor actions.", icon: LockKeyhole },
  { title: "Integrations", body: "CRM, webhook, export, and enterprise integrations remain disabled unless configured.", icon: Settings },
  { title: "Privacy", body: "Asset visibility, sponsor profile visibility, and public/private brand preferences.", icon: ShieldCheck },
