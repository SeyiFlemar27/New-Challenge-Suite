...
export default function EnterpriseApplicationPage() {
  ...
  useEffect(() => { void apiRequest<{ application: Application | null; canEdit?: boolean; canResubmit?: boolean }>("/api/enterprise-inquiries").then((result) => { setLoading(false); if (!result.ok) return setError(result.message); setApplication(result.data?.application ?? null); setCanEdit(Boolean(result.data?.canEdit || result.data?.canResubmit)); }); }, []);
