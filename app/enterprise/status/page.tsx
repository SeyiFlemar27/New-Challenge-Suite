...
export default function EnterpriseStatusPage() {
  ...
  function load() { void apiRequest<{ application: Application | null; timeline?: Array<{ type: string; status: string; at: string; message: string }>; canEdit?: boolean; canResubmit?: boolean }>("/api/enterprise-inquiries").then((result) => { setLoading(false); if (!result.ok) return setError(result.message); setApplication(result.data?.application ?? null); setTimeline(result.data?.timeline ?? []); setCanEdit(Boolean(result.data?.canEdit || result.data?.canResubmit)); }); }
  useEffect(load, []);
  ...
  const message = status === "approved" ? "You can now access your Enterprise workspace." : needsInfo ? "Please update your application so we can continue the review." : status === "rejected" ? canEdit ? "You can revise this application and submit it for another review." : "This decision is final unless an administrator changes your reapplication eligibility." : withdrawn ? canEdit ? "You can revise and reapply when your team is ready." : "Reapplication is not currently available." : "We'll notify you when your application has been reviewed.";
