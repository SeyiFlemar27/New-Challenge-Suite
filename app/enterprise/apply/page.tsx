...
export default function EnterpriseApplyPage() {
  ...
  async function submit() {
    setSubmitting(true);
    const result = await apiRequest("/api/enterprise-inquiries", { method: editId ? "PATCH" : "POST", body: JSON.stringify({ ...form, id: editId || undefined, company: form.organization, useCase: form.reason, applicationType: "enterprise_access_application" }) });
    setSubmitting(false);
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_360px] lg:items-start">
      <div>
        ...
        <Card className="mt-8 p-5 sm:p-7 lg:p-8">
          ...
          <Button className="mt-6" onClick={submit} disabled={submitting}>{submitting ? "Submitting..." : editId ? "Resubmit Application" : "Submit Application"}</Button>
        </Card>
      </div>
