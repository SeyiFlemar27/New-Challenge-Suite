...
const initialForm = { prizeName: "", prizeDescription: "", prizeTier: "basic", prizeType: "reward_points", rewardValue: "", maximumDiscountCents: "", entitlementExpiresAt: "", status: "draft", totalQuantity: "", deliveryCountries: "", fulfillmentInstructions: "", shippingPolicy: "", customsPolicy: "", budgetId: "", terms: "Rewards are subject to the configured fulfillment terms." };
...
export default function AdminPrizeCatalogPage() {
  ...
  async function createPrize() {
    setBusy(true);
    const response = await apiRequest("/api/admin/rewards/prizes", { method: "POST", body: JSON.stringify({ ...form, rewardValue: creatorBoost ? 3 : Number(form.rewardValue), unit: creatorBoost ? "days" : undefined, entitlementExpiresAt: form.entitlementExpiresAt ? new Date(form.entitlementExpiresAt).toISOString() : null, totalQuantity: physical ? Number(form.totalQuantity) : null, quantityType: physical ? "limited" : "unlimited", deliveryCountries: form.deliveryCountries.split(",").map((value) => value.trim().toUpperCase()).filter(Boolean), enabled: form.status === "active" }) });
