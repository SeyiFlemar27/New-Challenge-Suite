...
export default function EconomyRulesPage() {
  ...
  return <AdminShell>
    ...
    <Card className="mt-6 p-6"><h2 className="text-xl font-black">Economy monitoring</h2><p className="mt-2 text-sm text-slate-600">Counts and volumes come from stored ledger and Action Centre records. No synthetic metrics are shown.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="DoroCoin ledger entries" value={monitoring.doroCoinTransactionCount ?? 0} /><Metric label="Challenge Credit entries" value={monitoring.challengeCreditTransactionCount ?? 0} /><Metric label="Paid vote holds" value={monitoring.paidVoteHoldCount ?? 0} /><Metric label="Unresolved host/sponsor shares" value={monitoring.unresolvedHostSponsorAllocationCount ?? 0} /><Metric label="Growth Wallet entries" value={monitoring.growthWalletTransactionCount ?? 0} /><Metric label="Open economy tasks" value={monitoring.openEconomyActionTaskCount ?? 0} /></div></Card>
