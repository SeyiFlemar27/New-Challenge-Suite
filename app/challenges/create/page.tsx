...
function CreateChallengeWizard() {
  ...
  const planAccess = getUserPlanAccess({ planId: user?.planId });
...
function StepFormat({ form, update, toggleSubmission }: { form: any; update: any; toggleSubmission: (type: string) => void }) {
  return <section><h2 className="text-xl font-black sm:text-2xl">Step 2: Format & Rules</h2><div className="mt-6 grid gap-5 md:grid-cols-2"><Field label="Competition Format"><select className={inputClass} value={form.competitionFormat} onChange={(event) => update("competitionFormat", event.target.value)}><option>1 vs 1 Battle</option><option>Group Challenge</option><option>Entry Competition</option></select></Field><Field label="Best Of"><select className={inputClass} value={form.bestOf} onChange={(event) => update("bestOf", event.target.value)}><option>1 Rounder</option><option>Best of 3</option><option>Best of 5</option></select></Field></div><div className="mt-6"><div className="mb-2 font-bold">Submission Type</div><div className="grid gap-3 sm:grid-cols-2">{["image", "video"].map((type) => <label key={type} className="flex min-h-14 items-center gap-3 rounded-[8px] border border-white/10 bg-[#181818] px-4 py-4 font-bold"><input className="shrink-0" type="checkbox" checked={form.submissionTypes.includes(type)} onChange={() => toggleSubmission(type)} /> {type === "image" ? "Image uploads" : "Video uploads"}</label>)}</div></div><div className="mt-6 grid gap-4 md:grid-cols-2">{["Time limit uploads", "Standard platform rules", "Voting policy acknowledgement", "Editable rules enabled"].map((rule) => <label key={rule} className="flex items-start gap-3 font-bold"><input className="mt-1 shrink-0" type="checkbox" defaultChecked /> <span>{rule}</span></label>)}</div></section>;
}
