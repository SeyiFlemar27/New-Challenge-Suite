...
import { validateChallengeForPublish } from "@/lib/server/challenge-validation";
...
export default function ChallengeDraftPreviewPage() {
  ...
  const readiness = useMemo(() => validateChallengeForPublish(draft ?? {}, { mode: "publish", userId: user?.uid }), [draft, user?.uid]);
  const plan = getUserPlanAccess({ planId: user?.planId, planStatus: user?.planStatus, accountType: user?.accountType });
  const enterpriseApproved = plan.isEnterprise && String((user as any)?.enterpriseAccessStatus ?? (user as any)?.enterpriseApprovalStatus ?? "").toLowerCase() === "approved";
  const monetization = draft?.monetization && typeof draft.monetization === "object" ? draft.monetization as Record<string, unknown> : {};
  const paidEntryRequested = monetization.paidEntryRequested === true;
  const mediaStatus = String(draft?.mediaUploadStatus ?? draft?.mediaStatus ?? "");
  const publishBlocker = getChallengePublishBlocker({ authenticated: Boolean(user?.uid), ownsChallenge: !draft || String(draft.creatorId ?? draft.ownerId ?? "") === String(user?.uid ?? ""), status: String(draft?.status ?? draft?.lifecycleStatus ?? "draft"), planAllowsChallenge: !draft || !(paidEntryRequested || monetization.sponsorReady === true || monetization.prizePoolRequested === true || monetization.paidVotesRequested === true) || plan.isCreator || plan.isHost || enterpriseApproved, validation: readiness, mediaMissing: Boolean(draft && !draft.usesPlaceholderMedia && (!draft.coverImageUrl || !draft.coverImagePath)), mediaProcessing: ["preparing", "uploading", "processing"].includes(mediaStatus), mediaFailed: mediaStatus === "failed", paidEntryRequested, entryFeeValid: !paidEntryRequested || Number(monetization.entryFeeAmountCents ?? 0) >= 500 });
  async function publish() { if (!draft) return; if (publishBlocker) return setError(publishBlocker.message); setPublishing(true); setError(""); const result = await publishChallengeDraft(draftId, { ...draft, publish: true }); setPublishing(false); if (!result.ok) return setError(challengePublishError(result)); setSubmitted(true); }
  ...
  return <AppShell>
    ...
    {error ? <div className="mx-auto max-w-[1240px]"><ApiErrorPanel title="Challenge could not be submitted" message={error} onRetry={() => setError("")} /></div> : null}
    {draft && publishBlocker && !error ? <Card className="mx-auto mb-5 max-w-[1240px] border-yellow-500/30 bg-yellow-500/5 p-4 text-sm text-slate-200">{publishBlocker.message}</Card> : null}
