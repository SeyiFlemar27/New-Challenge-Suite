...
function sanitizeDraftPatch(body: Record<string, unknown>) {
  ...
  if (patch.challengeType === "normal" || patch.builderVersion === "normal_v2") {
    patch.participationMode = "open";
    patch.requiresParticipantApproval = false;
    patch.participantApprovalMode = "automatic";
  }
  if (patch.isLiveEvent === true && (patch.maxParticipants !== undefined || patch.capacityMode !== undefined)) {
    patch.eventCapacity = patch.maxParticipants;
  }
