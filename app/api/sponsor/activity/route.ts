...
export async function GET(request: Request) {
  ...
  const db = getAdminDb();
  if (!db) return serverUnavailable("Sponsor activity");
  try {
    const sponsor = await loadSponsor(db, user.uid);
    if (!sponsor) return forbidden("A sponsor account is required.");
    const snap = await db.collection("sponsorActivity").where("userId", "==", user.uid).orderBy("createdAt", "desc").limit(50).get();
    return ok({ activity: snap.docs.map((doc) => ({ id: doc.id, ...doc.data() })) }, "Sponsor activity loaded.");
  } catch (error) {
    console.error("[sponsor-activity:get]", { userId: user.uid, message: error instanceof Error ? error.message : String(error) });
    return serverError("Sponsor activity could not be loaded.");
  }
}