...
export async function GET(request: Request) {
  ...
  try {
    const walletSnap = await context.db.collection("sponsorWallets").doc(context.user.uid).get();
    const [transactionsSnap, fundingSnap] = await Promise.all([
      context.db.collection("sponsorWalletTransactions").where("sponsorId", "==", context.user.uid).limit(25).get(),
      context.db.collection("sponsorCampaignFunding").where("sponsorId", "==", context.user.uid).limit(25).get()
    ]);
    const wallet = walletSnap.exists ? { id: walletSnap.id, ...walletSnap.data() } : { id: context.user.uid, sponsorId: context.user.uid, availableBalanceCents: 0, reservedFundsCents: 0, pendingTransactionsCents: 0, campaignCommitmentsCents: 0, releasedPaymentsCents: 0, refundsCents: 0, failedPaymentsCents: 0, totalSpendCents: 0, currency: "USD", providerStatus: "not_configured", fundingEnabled: false, paymentReleaseEnabled: false };
