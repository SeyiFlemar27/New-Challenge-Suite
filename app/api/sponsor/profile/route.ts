...
async function persistSponsorProfile(request: Request) {
  ...
  try {
    ...
    await Promise.all([
      ...
      db.collection("sponsorBrandAssets").doc(user.uid).set({ userId: user.uid, logoUrl: finalSponsorProfile.logoUrl, alternateLogoUrl: finalSponsorProfile.alternateLogoUrl, squareIconUrl: finalSponsorProfile.squareIconUrl, bannerUrl: finalSponsorProfile.bannerUrl, coverImageUrl: finalSponsorProfile.coverImageUrl, brandColors: finalSponsorProfile.brandColors, brandFonts: finalSponsorProfile.brandFonts, updatedAt: now, updatedBy: user.uid }, { merge: true }),
