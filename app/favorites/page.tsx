  ...
  return (
    <AppShell>
      ...
      {!query.isLoading && query.data?.ok && items.length ? <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => {
        ...
        return <Card key={item.engagementId} className="overflow-hidden"><div className="h-44 bg-[#191919] bg-cover bg-center" style={imageUrl ? { backgroundImage: `url(${imageUrl})` } : undefined} /><div className="p-5"><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--gold)]">{active.label}</p><h2 className="mt-3 break-words text-xl font-black">{title}</h2>{tab === "interested" ? <p className="mt-3 text-sm leading-6 text-slate-400">In-app reminders: {item.reminderOffsetsMinutes.length ? "1 hour, 30 minutes, 5 minutes, and start time" : "not configured"}. Push/email delivery is not active yet.</p> : null}<div className="mt-5 grid gap-2 sm:grid-cols-2"><LinkButton href={tab === "interested" || tab === "watchLater" ? `/challenges/${challengeId}/watch` : `/challenges/${challengeId}`} className="w-full">{tab === "saved" ? "View Challenge" : "Open Watch Details"}</LinkButton><Button variant="ghost" onClick={() => mutation.mutate({ challengeId, action: removeAction })} disabled={mutation.isPending}><Trash2 size={16} /> Remove</Button></div></div></Card>;
      })}</div> : null}
