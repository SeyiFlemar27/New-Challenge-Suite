...
export default function HostChallengesPage() {
  const { data, isLoading } = useQuery({ queryKey: ["dashboard", "host-challenges"], queryFn: fetchDashboard, staleTime: 30_000 });
  ...
  return <PlanFeatureGate feature="host_control_center" requiredPlan="Host" title="Host tools are available on the Host Plan."><AppShell><div className="mx-auto max-w-7xl">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><PageTitle title="Challenges" subtitle="Manage public challenges created by your host workspace." icon={<Medal />} /><LinkButton href="/challenges/create" className="w-full sm:w-auto">Create Challenge</LinkButton></div>
