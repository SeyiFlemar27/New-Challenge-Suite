...
export default function HostTournamentsPage() {
  ...
  return <PlanFeatureGate feature="tournament_builder" requiredPlan="Host" title="Tournament tools require Host Plan"><AppShell><div className="mx-auto max-w-7xl">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><PageTitle title="Tournaments" subtitle="Manage tournament challenges, rounds, and final-stage planning." icon={<Trophy />} /><LinkButton href="/host/tournaments/create" className="w-full sm:w-auto">Create Tournament Challenge</LinkButton></div>
    ...
    {!isLoading && !errorMessage && !tournaments.length ? <Card className="mt-8"><EmptyState icon={<Trophy className="text-[var(--gold)]" />} title="No tournaments yet" body="Tournament challenges you create or manage will appear here." action={<LinkButton href="/host/tournaments/create">Create Tournament Challenge</LinkButton>} /></Card> : null}
