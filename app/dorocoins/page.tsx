...
export default function DoroCoinsPage() {
  ...
  return <AppShell>
    <div className="mx-auto w-full max-w-7xl">
      ...
      <section className="mt-7 overflow-hidden rounded-[8px] border border-yellow-400/35 bg-[linear-gradient(135deg,#171717,#282000)] p-6 text-white sm:p-8">
        <div className="grid gap-7 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-sm font-bold text-yellow-300">Available DoroCoin balance</p><p className="mt-2 text-5xl font-black tracking-normal text-yellow-300 sm:text-6xl">{loading ? "..." : balance.toLocaleString()}</p><p className="mt-4 max-w-2xl text-sm leading-6 text-white/70">DoroCoins are a virtual platform currency. They are not cash, are not legal tender, and cannot currently be withdrawn or converted to cash.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:w-[360px]"><Button onClick={() => chooseTab("earn")}>Earn DoroCoins</Button><Button variant="secondary" className="border-white/30 text-white" onClick={() => chooseTab("buy")}>Buy DoroCoins</Button><Button variant="ghost" className="border-white/20 bg-white/5 text-white" onClick={() => chooseTab("transfer")}>Transfer</Button><Button variant="ghost" className="border-white/20 bg-white/5 text-white" onClick={() => chooseTab("history")}>View History</Button></div></div>
      </section>
