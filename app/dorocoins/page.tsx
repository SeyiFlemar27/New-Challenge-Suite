...
type DoroTransaction = { id: string; type?: string; status?: string; description?: string; amount?: number; createdAt?: string | null };
type AdAvailability = { available: boolean; rewardAmount: number; adConsecutiveLimit: number; adCooldownMinutes: number };
type EconomySummary = { balances: { cash: { availableCents: number }; doroCoins: { balance: number }; challengeCredits: { balance: number }; creatorGrowthWallet: { balanceCents: number } }; streak: { current: number; awardedMilestones: number[] }; doroCoinActivity?: { earnedToday: number; pendingOrReviewCount: number; reversedCount: number } };
...
export default function DoroCoinsPage() {
  ...
  return <AppShell>
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-3"><Quote label="Earned today" value={`${economy?.doroCoinActivity?.earnedToday ?? 0} DoroCoins`}/><Quote label="Pending or reviewed" value={`${economy?.doroCoinActivity?.pendingOrReviewCount ?? 0}`}/><Quote label="Reversed entries" value={`${economy?.doroCoinActivity?.reversedCount ?? 0}`}/></div>
    ...
    <div className="mt-8 grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
      ...
      <Card className="p-6 sm:p-8">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3"><Quote label="Per verified ad" value={`${ads?.rewardAmount ?? 5} coins`} /><Quote label="Cycle limit" value={`${ads?.adConsecutiveLimit ?? 10} ads`} /><Quote label="Cooldown" value={`${ads?.adCooldownMinutes ?? 120} min`} /></div>
    ...
    <section className="mt-10"><h2 className="text-2xl font-black">Ways to earn and spend</h2><div className="mt-4 grid gap-4 lg:grid-cols-2"><Card className="p-5"><h3 className="font-black">Earn DoroCoins</h3><div className="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-2"><Reward label="Daily login" value="10 daily"/><Reward label="Challenge video" value="2, max 20/day"/><Reward label="Like a challenge" value="1, max 50/day"/><Reward label="Valid comment" value="3, max 20/day"/><Reward label="Share a challenge" value="5, max 10/day"/><Reward label="Verified referral" value="50 once"/><Reward label="Create a free challenge" value="25 once"/><Reward label="Join a free challenge" value="20 once"/><Reward label="Win a free challenge" value="150 once"/><Reward label="Confirmed top 10" value="75 once"/><Reward label="Profile verification" value="100 once"/><Reward label="Verified sponsored ad" value="5-10"/></div><p className="mt-4 text-xs leading-5 text-slate-500">Only server-validated events earn rewards. Capped, reversed, or reviewed activity is reflected in your ledger.</p></Card><Card className="p-5"><h3 className="font-black">Use DoroCoins</h3><p className="mt-2 text-sm leading-6 text-slate-600">Eligible free community entries, visibility boosts, configured raffle entries, profile cosmetics, badges, gifts, seasonal events, education, and promotional items. Additional votes use Challenge Credits, not DoroCoins.</p></Card></div></section>
    ...
    <section className="mt-10">
      ...
      <Card className="mt-5 overflow-hidden">{transactions.length ? transactions.slice(0, 20).map((item) => <div key={item.id} className="grid gap-2 border-b border-white/10 p-5 sm:grid-cols-[140px_1fr_120px] sm:items-center"><span className="text-sm text-slate-400">{item.createdAt ? new Date(item.createdAt).toLocaleDateString() : "Pending"}</span><span><b>{item.description || formatType(item.type)}</b><span className="mt-1 block text-xs text-slate-500">{formatType(item.type)} - {formatType(item.status ?? "confirmed")}</span></span><span className={`text-lg font-black sm:text-right ${Number(item.amount) >= 0 ? "text-emerald-300" : "text-red-300"}`}>{Number(item.amount) > 0 ? "+" : ""}{Number(item.amount || 0)}</span></div>) : <EmptyState icon={<Vote />} title="No DoroCoin activity yet" body="Confirmed purchases and eligible DoroCoin usage will appear here." />}</Card>
    </section>
...
function Reward({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-3 rounded-[8px] border border-black/10 p-3"><span>{label}</span><strong>{value}</strong></div>; }
