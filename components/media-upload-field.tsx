    ...
    try {
      ...
      const downloadUrl = await new Promise<string>((resolve, reject) => {
        ...
        uploadTask.on("state_changed", (snapshot) => {
          setProgress(Math.round((snapshot.bytesTransferred / Math.max(snapshot.totalBytes, 1)) * 100));
        }, (caught) => {
  ...
  return (
    <div>
      ...
      <Card className="border-white/10 bg-black/30 p-4">
        ...
        {uploading ? <div className="mt-4"><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-[var(--gold)] transition-all" style={{ width: `${progress}%` }} /></div><p className="mt-2 text-xs font-bold text-slate-400">Uploading {progress}%</p></div> : null}
        {value ? <div className="mt-4 flex flex-wrap gap-3"><Button type="button" variant="secondary" onClick={() => inputRef.current?.click()}><UploadCloud size={16} /> Replace</Button><Button type="button" variant="ghost" onClick={() => onChange("")}><Trash2 size={16} /> Remove</Button></div> : null}
