...
const PUBLIC_TAGLINE="Every Challenge Starts Here. Battle. Compete. Dominate. Create.";
const footerGroups=[
...
function dashboardHref(user:ReturnType<typeof useCurrentUser>["user"]){if(!user)return"/dashboard";if(user.isSponsor)return user.sponsorOnboardingComplete?"/sponsor/dashboard":user.hasSponsorProfile?"/sponsor/onboarding":"/sponsor/start";return"/dashboard"}
...
export function PublicHeader({announcement}: {announcement?:Announcement}){
 const {t}=useLanguage();
 const {user}=useCurrentUser(); const [open,setOpen]=useState<string|null>(null);const [drawer,setDrawer]=useState(false);const [expanded,setExpanded]=useState<string|null>(null);const drawerRef=useRef<HTMLDivElement>(null);const accountHref=dashboardHref(user);
 ...
 return <><header className="public-header" data-public-header><div className="public-container flex h-[76px] items-center justify-between gap-6">
  ...
  <div className="hidden items-center gap-2 lg:flex"><LanguageSelector compact persistAccount={Boolean(user)} />{user?<><Link href="/profile" className="public-login">Profile</Link><Link href={accountHref} className="public-primary-button">Dashboard</Link></>:<><Link href="/auth/login" className="public-login">{t("signIn")}</Link><Link href="/auth/register" className="public-primary-button">{t("signUp")}</Link></>}</div>
 ...
 {announcement?<div className="public-container py-3"><div className="public-announcement"><p>{PUBLIC_TAGLINE}</p><Link href={announcement.ctaRoute}>{announcement.ctaLabel}<ChevronRight size={16}/></Link></div></div>:null}
 {drawer?<div className="fixed inset-0 z-[100]" role="dialog" aria-modal="true" aria-label="Public navigation"><button type="button" className="absolute inset-0 bg-black/45" onClick={()=>setDrawer(false)} aria-label="Close navigation"/><div ref={drawerRef} className="public-drawer">
  ...
  <div className="mt-8 grid gap-3"><LanguageSelector persistAccount={Boolean(user)} />{user?<><Link href={accountHref} className="public-primary-button justify-center" onClick={()=>setDrawer(false)}>Dashboard</Link><Link href="/profile" className="public-secondary-button justify-center" onClick={()=>setDrawer(false)}>Profile</Link></>:<><Link href="/auth/register" className="public-primary-button justify-center" onClick={()=>setDrawer(false)}>Sign up</Link><Link href="/auth/login" className="public-secondary-button justify-center" onClick={()=>setDrawer(false)}>Sign in</Link></>}</div>
 </div></div>:null}</>
}
export function PublicFooter(){
 ...
 <div className="flex flex-col gap-5 py-8 text-sm text-white/60 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3"><PublicLogo size={36}/><span>Challenge Suite. Competition, made intentional.</span></div><div className="flex flex-wrap items-center gap-4"><LanguageSelector compact /><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><Link href="/cookie-policy">Cookie preferences</Link><span>Mobile apps coming soon</span></div></div>
 </div></footer>
}
