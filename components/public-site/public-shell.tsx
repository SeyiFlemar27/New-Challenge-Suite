...
export function PublicHeader({announcement}: {announcement?:Announcement}){
 ...
 return <><header className="public-header" data-public-header><div className="public-container flex h-[76px] items-center justify-between gap-6">
  ...
  <div className="hidden items-center gap-2 lg:flex"><LanguageSelector compact persistAccount={Boolean(user)} /><Link href={signInHref} className="public-login">{t("signIn")}</Link><Link href={signUpHref} className="public-primary-button">{t("signUp")}</Link></div>
 ...
 {drawer?<div className="fixed inset-0 z-[100]" role="dialog" aria-modal="true" aria-label="Public navigation"><button type="button" className="absolute inset-0 bg-black/45" onClick={()=>setDrawer(false)} aria-label="Close navigation"/><div ref={drawerRef} className="public-drawer">
  ...
  <div className="mt-8 grid gap-3"><LanguageSelector persistAccount={Boolean(user)} /><Link href={signUpHref} className="public-primary-button justify-center">Sign up</Link><Link href={signInHref} className="public-secondary-button justify-center">Sign in</Link></div>
 </div></div>:null}</>
}
