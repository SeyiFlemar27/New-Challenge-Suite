...
function WizardStep({ step, form, update, userId }: { step: number; form: Form; update: <K extends keyof Form>(key: K, value: Form[K]) => void; userId: string }) {
  ...
  if (step === 8) return <Step title="Media & Branding"><Grid><MediaUploadField label="Promo Flyer" value={form.promoImageUrl} onChange={(url) => update("promoImageUrl", url)} storagePath={`eventMedia/host-drafts/${userId}/promo-flyer`} kind="image" buttonLabel="Upload Promo Image" /><MediaUploadField label="Trailer Video" value={form.trailerVideoUrl} onChange={(url) => update("trailerVideoUrl", url)} storagePath={`eventMedia/host-drafts/${userId}/trailer`} kind="video" buttonLabel="Upload Trailer Video" /><MediaUploadField label="Event Logo" value={form.eventLogoUrl} onChange={(url) => update("eventLogoUrl", url)} storagePath={`eventMedia/host-drafts/${userId}/logo`} kind="image" buttonLabel="Upload Event Logo" /><MediaUploadField label="Sponsor Banner" value={form.sponsorBannerUrl} onChange={(url) => update("sponsorBannerUrl", url)} storagePath={`eventMedia/host-drafts/${userId}/sponsor-banner`} kind="image" buttonLabel="Upload Sponsor Banner" /><Field label="Brand Color"><input className={inputClass} type="color" value={form.brandColor} onChange={(e) => update("brandColor", e.target.value)} /></Field></Grid><p className="text-sm text-slate-400">Media uploads fail closed until secure Storage rules are manually published. External livestream links remain partner URLs only.</p></Step>;
