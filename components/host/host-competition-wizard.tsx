...
function SpecializedFlowNotice({ competitionType }: { competitionType: string }) {
  ...
  return <Card className="p-4 text-sm leading-6 text-slate-300">Private challenges, live events, tournaments, and hybrid competitions use dedicated workspace entries for their format.</Card>;
}
...
function HybridCompetitionBuilder() {
  ...
  if (createdId) return <AppShell><Card className="mx-auto max-w-2xl p-8 text-center"><CheckCircle2 className="mx-auto h-14 w-14 text-emerald-400" /><h1 className="mt-5 text-3xl font-black">Hybrid competition saved</h1><p className="mt-3 text-slate-300">Online qualification, finalist selection, final round, judging, and winner review have been saved.</p><div className="mt-6 flex justify-center gap-3"><LinkButton href={createdId === "saved" ? "/challenges" : `/challenges/${createdId}`}>View Competition</LinkButton><LinkButton href="/host/hybrid" variant="secondary">Hybrid Workspace</LinkButton></div></Card></AppShell>;
  return <AppShell><div className="mx-auto max-w-6xl" data-mobile-creator-management><PageTitle title="Create Hybrid Competition" subtitle="Hybrid competitions combine online submissions with a live, scheduled, or judged final stage. Use this when participants qualify online before advancing to a final round." /><Card className="mt-6 p-4 sm:p-6"><div className="grid gap-2 sm:grid-cols-2 lg:flex" data-mobile-builder-steps>{hybridSteps.map((label, index) => <button key={label} onClick={() => index <= step && setStep(index)} className={`min-h-11 rounded-[8px] px-3 text-left text-xs font-black lg:min-w-40 ${index === step ? "bg-[var(--gold)] text-black" : "bg-[#191919] text-slate-300"}`}>{index + 1}. {label}</button>)}</div><div className="mt-8 min-h-[470px]"><HybridStep step={step} form={form} update={update} userId={auth.user?.uid ?? "anonymous"} /></div>{error ? <p className="mt-5 rounded-[8px] bg-red-950/50 p-4 text-red-200">{error}</p> : null}<div className="mt-8 flex flex-col gap-3 border-t border-white/10 pt-6 sm:flex-row sm:justify-between"><Button variant="secondary" onClick={() => submit(false)} disabled={saving}><Save size={17} /> Save Draft</Button><div className="grid grid-cols-2 gap-3"><Button variant="ghost" disabled={step === 0} onClick={() => setStep((value) => value - 1)}>Back</Button>{step < hybridSteps.length - 1 ? <Button onClick={() => { const problem = validate(); problem ? setError(problem) : setStep((value) => value + 1); }}>Next</Button> : <Button onClick={() => submit(form.launchMode !== "draft")} disabled={saving}>{saving ? "Saving..." : "Publish Hybrid"}</Button>}</div></div></Card></div></AppShell>;
}
