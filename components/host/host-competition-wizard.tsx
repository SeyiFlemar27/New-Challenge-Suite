...
const liveEventStepMap = [0, 3, 3, 3, 4, 5, 7, 8, 9, 10] as const;
...
export function HostCompetitionWizard({ initialCompetitionType }: { initialCompetitionType?: string } = {}) {
  ...
  const payload = (publish: boolean) => {
    ...
    return {
      ...
      votingSettings: { allowFreeVotes: form.allowFreeVotes, allowPaidVotes: form.allowDoroCoinVotes, weightedVotes: form.weightedVotes },
      ...
      hostOperations: {
        ...
        ticketInstructions: live ? form.ticketInstructions : "", checkInTime: live ? form.checkInTime : "", checkInRequired: live && form.checkInRequired,
        ticketCheckoutActive: false, ticketPaymentConfirmationRequired: true
      }, publish
    };
  };
  ...
  const actions = <div className="mt-8 flex flex-col gap-3 border-t border-white/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="grid gap-3 sm:flex">
      ...
      {step < activeSteps.length - 1 ? <Button onClick={() => { const problem = validate(); problem ? setError(problem) : setStep((value) => value + 1); }}>Continue</Button> : <Button onClick={() => submit(form.launchMode !== "draft")} disabled={saving}>{saving ? "Saving..." : form.launchMode === "schedule" ? "Schedule Launch" : "Publish Competition"}</Button>}
    </div>
  </div>;
    ...
    return <AppShell><div className="mx-auto max-w-[1440px]">
      ...
      <div className="mt-7 grid items-start gap-6 lg:grid-cols-[240px_minmax(0,1fr)_280px]">
        ...
        <Card className="min-w-0 p-5 sm:p-7">
          <div className="min-h-[470px]"><WizardStep step={liveEventStepMap[step] ?? 10} form={form} update={update} userId={auth.user?.uid ?? "anonymous"} /></div>
