...
import { BarChart3, Bookmark, ClipboardCheck, FileText, Handshake, LayoutDashboard, LifeBuoy, LogOut, Menu, Search, Settings, WalletCards, X } from "lucide-react";
...
export function SponsorShell({ children, profile }: { children: React.ReactNode; profile?: SponsorShellProfile | null }) {
  ...
  const groups: NavGroup[] = [
    { label: "Main", items: [
      { label: "Sponsor Studio", href: "/sponsor/dashboard", icon: LayoutDashboard },
      { label: "Discover", href: "/sponsor/discover", icon: Search },
  ...
  const nav = <nav className="mt-6 grid gap-5" aria-label="Sponsor workspace">{groups.map((group) => <section key={group.label}><p className="px-3 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">{group.label}</p><div className="mt-1 grid gap-1">{group.items.map(navLink)}</div></section>)}</nav>;
  const statusCard = !workspace.approved ? <Card className="mt-6 border-amber-200 bg-amber-50/70 p-4 shadow-none"><p className="text-[11px] font-black uppercase tracking-[0.18em] text-amber-800">Sponsor status</p><p className="mt-2 text-sm font-black text-slate-950">{workspace.statusLabel}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-[var(--gold)]" style={{ width: `${workspace.completionPercent}%` }} /></div><Link href="/sponsor/onboarding" className="mt-3 inline-flex text-xs font-black text-amber-800 hover:text-slate-950">Complete brand profile</Link></Card> : null;
  const accountFooter = <div className="mt-6 border-t border-slate-200 pt-5"><WorkspaceSwitcher user={user} /><div className="mt-3 grid gap-1"><Link href="/sponsor/profile" className="rounded-[8px] px-3 py-2 text-sm font-bold text-slate-700 hover:bg-amber-50">Brand Profile</Link><Link href="/sponsor/settings?section=channels" className="rounded-[8px] px-3 py-2 text-sm font-bold text-slate-700 hover:bg-amber-50">Connected Channels</Link><Link href="/sponsor/billing" className="rounded-[8px] px-3 py-2 text-sm font-bold text-slate-700 hover:bg-amber-50">Plan &amp; Billing</Link><button type="button" onClick={() => void logout().finally(() => { window.location.href = "/auth/login"; })} className="flex items-center gap-2 rounded-[8px] px-3 py-2 text-left text-sm font-bold text-slate-700 hover:bg-amber-50"><LogOut size={15} /> Sign Out</button></div></div>;
  ...
  return <main className="sponsor-mobile-shell theme-workspace min-h-screen overflow-x-hidden bg-[var(--background)] text-[var(--foreground)]">
    ...
    <div className="grid min-h-screen md:grid-cols-[260px_minmax(0,1fr)] xl:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <section className="min-w-0 px-4 pb-12 pt-5 sm:px-6 md:px-8 md:py-8 lg:px-10 xl:px-12">
        ...
        {drawerOpen ? <div className="fixed inset-0 z-[90] md:hidden" role="dialog" aria-modal="true" aria-label="Sponsor navigation"><button className="absolute inset-0 bg-slate-950/50" onClick={() => setDrawerOpen(false)} aria-label="Close sponsor navigation" /><aside className="absolute bottom-0 right-0 top-0 w-[min(90vw,360px)] overflow-y-auto border-l border-slate-200 bg-white p-5 shadow-xl"><div className="flex items-center justify-between"><p className="text-lg font-black text-slate-950">{brandName}</p><button onClick={() => setDrawerOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-[8px] border border-slate-200 text-slate-700" aria-label="Close menu"><X /></button></div>{statusCard}{nav}{accountFooter}<Link href="/landing" onClick={() => setDrawerOpen(false)} className="mt-6 block rounded-[8px] border border-slate-200 p-3 text-center font-bold text-slate-700">Public site</Link></aside></div> : null}
