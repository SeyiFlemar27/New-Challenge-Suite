...
type FormState = {
  ...
  sponsorshipGoal: string; preferredSponsorCategories: string[]; sponsorNote: string; sponsorPlacementPreferences: string[];
};
...
function formFromChallenge(challenge: Record<string, unknown>): FormState {
  ...
  return {
    ...
    preferredSponsorCategories: normalizeSponsorCategories(monetization.preferredSponsorCategories ?? monetization.preferredSponsorCategory),
  ...
  function payload(publish: boolean) {
    ...
    return {
      ...
      monetization: {
        ...
        preferredSponsorCategory: safeSponsorReady ? form.preferredSponsorCategories[0] ?? "" : "",
        preferredSponsorCategories: safeSponsorReady ? normalizeSponsorCategories(form.preferredSponsorCategories) : [],
      ...
      <div className="space-y-5">
        ...
        <MonetizationCard title="Make this challenge Sponsor Ready" enabled={form.sponsorReady} disabled={!monetizationEligible} onChange={(enabled) => update("sponsorReady", enabled)} setupCopy="Sponsor-ready challenges can appear in Sponsor Discovery after publish. Confirmed sponsor funds remain separate from generated revenue and stay directed to their approved sponsor purpose.">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <MultiSelect label="Preferred sponsor categories" values={form.preferredSponsorCategories} options={SPONSOR_CATEGORY_OPTIONS} maximum={3} disabled={!form.sponsorReady || !monetizationEligible} onChange={(values) => update("preferredSponsorCategories", values)} />
          </div>
