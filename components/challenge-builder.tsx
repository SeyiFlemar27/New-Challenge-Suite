...
function MediaBrandingStep({ form, base, updateMedia, track, mediaUploadDisabled, mediaUploadDisabledReason }: { form: FormState; base: string; updateMedia: (urlField: keyof FormState, pathField: keyof FormState, url: string, metadata?: { path: string }) => void; track: (field: string) => (status: MediaUploadStage) => void; mediaUploadDisabled: boolean; mediaUploadDisabledReason: string }) {
  return <section><StepTitle title="Media & Branding" body="Add real visuals for cards, promotion, and participant context." />{mediaUploadDisabled ? <Card className="mt-5 border-yellow-500/25 bg-yellow-500/5 p-4 text-sm leading-6 text-yellow-50"><b className="text-white">Media uploads are temporarily unavailable while storage is being connected.</b><br />You can publish this challenge without media for now. A branded Challenge Suite placeholder will appear in feeds and challenge cards.</Card> : null}<UploadGallery form={form} base={base} updateMedia={updateMedia} track={track} mediaUploadDisabled={mediaUploadDisabled} mediaUploadDisabledReason={mediaUploadDisabledReason} className="mt-6" /></section>;
}

function UploadGallery({ form, base, updateMedia, track, mediaUploadDisabled, mediaUploadDisabledReason, className = "" }: { form: FormState; base: string; updateMedia: (urlField: keyof FormState, pathField: keyof FormState, url: string, metadata?: { path: string }) => void; track: (field: string) => (status: MediaUploadStage) => void; mediaUploadDisabled: boolean; mediaUploadDisabledReason: string; className?: string }) {
  return <div className={`${className} space-y-7`}>
    <div>
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-3">
        <UploadPanel icon={<ImageIcon size={20} />} title="Image 1" purpose="Primary cover image used on challenge cards and detail pages." required={!mediaUploadDisabled}><MediaUploadField label="Cover image" value={form.coverImageUrl} onChange={(url, metadata) => updateMedia("coverImageUrl", "coverImagePath", url, metadata)} storagePath={base + "/banner"} kind="image" buttonLabel="Browse cover image" required={!mediaUploadDisabled} disabled={mediaUploadDisabled} disabledReason={mediaUploadDisabledReason} onStatusChange={track("coverImageUrl")} /></UploadPanel>
        <UploadPanel icon={<ImageIcon size={20} />} title="Image 2" purpose="Promo flyer / poster for campaign surfaces when supported."><MediaUploadField label="Promo flyer / poster" value={form.promoImageUrl} onChange={(url, metadata) => updateMedia("promoImageUrl", "promoImagePath", url, metadata)} storagePath={base + "/promo-flyer"} kind="image" buttonLabel="Browse promo asset" disabled={mediaUploadDisabled} disabledReason={mediaUploadDisabledReason} onStatusChange={track("promoImageUrl")} /></UploadPanel>
    ...
    <div>
      ...
      <div className="mt-4 grid gap-5 md:grid-cols-2">
        <DocumentSlot title="Document 1" mediaUploadDisabled={mediaUploadDisabled} />
        <DocumentSlot title="Document 2" mediaUploadDisabled={mediaUploadDisabled} />
      </div>
    </div>
  </div>;
}
...
function DocumentSlot({ title, mediaUploadDisabled = false }: { title: string; mediaUploadDisabled?: boolean }) {
  return <UploadPanel icon={<FileText size={20} />} title={title} purpose="Optional rules, brief, or reference document for participants when backend document storage is connected.">
    <div className="flex min-h-36 flex-col items-center justify-center rounded-[8px] border border-dashed border-white/15 bg-black/20 p-5 text-center">
      ...
      <p className="mt-3 text-sm font-bold text-slate-300">{mediaUploadDisabled ? "Document media skipped for now" : "Document upload setup required"}</p>
      <p className="mt-2 text-xs leading-5 text-slate-500">{mediaUploadDisabled ? "Documents are optional and no upload request will be attempted while storage is disabled." : "Documents are not published until backend document persistence is connected."}</p>
