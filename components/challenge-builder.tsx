...
import { CheckCircle2, Eye, LockKeyhole, Save, Sparkles, X } from "lucide-react";
...
function Stepper({ steps, current, onSelect }: { steps: string[]; current: number; onSelect: (step: number) => void }) {
  return <div className="flex gap-3 overflow-x-auto pb-1">{steps.map((label, index) => <button key={label} type="button" onClick={() => onSelect(index)} className={(index === current ? "border-[var(--gold)] bg-[var(--gold)] text-black" : index < current ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-100" : "border-white/10 bg-[#171717] text-slate-400") + " flex min-w-[170px] items-center gap-3 rounded-[8px] border px-4 py-3 text-left text-sm font-black"}><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-black/20">{index < current ? "✓" : index + 1}</span><span>{label}</span></button>)}</div>;
}
...
function StepContent({ mode, step, form, update, toggleType, updateMedia, track, userId, planAccess, planName }: { mode: Mode; step: number; form: FormState; update: (field: keyof FormState, value: string | string[]) => void; toggleType: (type: string) => void; updateMedia: (urlField: keyof FormState, pathField: keyof FormState, url: string, metadata?: { path: string }) => void; track: (field: string) => (status: MediaUploadStage) => void; userId: string; planAccess: ReturnType<typeof getUserPlanAccess>; planName: string }) {
  ...
  if (mode === "public" && step === 4) { const base = "challenges/drafts/" + userId; return <section><StepTitle title="Media & Branding" body="Add real visuals for the challenge preview." /><div className="mt-6 grid gap-6 md:grid-cols-2"><MediaUploadField label="Cover image" value={form.coverImageUrl} onChange={(url, metadata) => updateMedia("coverImageUrl", "coverImagePath", url, metadata)} storagePath={base + "/banner"} kind="image" buttonLabel="Upload Cover Image" required onStatusChange={track("coverImageUrl")} /><MediaUploadField label="Promo flyer" value={form.promoImageUrl} onChange={(url, metadata) => updateMedia("promoImageUrl", "promoImagePath", url, metadata)} storagePath={base + "/promo-flyer"} kind="image" buttonLabel="Upload Promo Image" onStatusChange={track("promoImageUrl")} /><MediaUploadField label="Trailer video" value={form.trailerVideoUrl} onChange={(url, metadata) => updateMedia("trailerVideoUrl", "trailerVideoPath", url, metadata)} storagePath={base + "/trailers"} kind="video" buttonLabel="Upload Trailer Video" onStatusChange={track("trailerVideoUrl")} /><Field label="Branding notes"><textarea className={textareaClass} placeholder="Optional visual notes for this challenge." /></Field></div></section>; }
  return <section><StepTitle title="Review & Publish" body="Check the challenge before creating a record." /><div className="mt-6 grid gap-4 md:grid-cols-2">{Object.entries({ Title: form.title || "Not set", Category: form.category || "Not set", Visibility: mode === "private" ? "Private / invite-only" : "Public", "Submission Types": form.submissionTypes.join(", "), "Submission Close": form.submissionDeadline, Plan: mode === "private" ? "Creator Plan" : planName }).map(([label, value]) => <Card key={label} className="p-4"><div className="text-sm font-bold text-slate-400">{label}</div><div className="mt-1 break-words text-base font-black text-white">{String(value)}</div></Card>)}</div><Card className="mt-5 p-4 text-sm leading-6 text-slate-300"><b className="text-white">Locked or excluded:</b> paid entry, cash prize release, sponsor money movement, livestream backend, advanced voting, revenue sharing, and automatic payouts are not enabled by this builder.</Card></section>;
}
