...
function Review({ form, readiness, edit }: { form: NormalChallengeForm; readiness: NormalChallengeReadiness; edit: (step: number) => void }) {
  ...
  return <><Heading step={6} /><div className="mt-9 divide-y divide-slate-200 border-y border-slate-200">{cards.map(([title, body, target]) => <section key={title} className="flex items-start justify-between gap-5 py-5"><div><h3 className="font-black text-slate-950">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{body}</p></div><Button variant="ghost" onClick={() => edit(target)}>Edit</Button></section>)}</div><div className={`mt-6 rounded-[8px] p-5 ${issues.length ? "bg-red-50" : "bg-emerald-50"}`}><h3 className="font-black text-slate-950">{issues.length ? `${issues.length} thing${issues.length === 1 ? "" : "s"} need to be fixed.` : "Ready to continue to Publish"}</h3>{issues.length ? <ul className="mt-3 space-y-2 text-sm text-red-800">{issues.map((item) => <li key={item.code}>- {item.message}</li>)}</ul> : <p className="mt-2 text-sm text-emerald-800">Complete the three confirmations before submitting for review.</p>}</div></>;
}
