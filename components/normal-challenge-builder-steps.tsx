...
import { challengeDateTimeForStorage, formatChallengeLocalDateTime } from "@/lib/challenge-date-time";
import { NORMAL_CHALLENGE_STEP_DEFINITIONS } from "@/lib/challenge-builder-foundation";
import { NORMAL_CHALLENGE_CATEGORIES, NORMAL_RESUBMIT_WINDOWS } from "@/lib/normal-challenge-config";
...
import { CHALLENGE_CATEGORY_OPTIONS, COUNTRY_OPTIONS, SPONSOR_CATEGORY_OPTIONS, TIMEZONE_OPTIONS, challengeSubcategoryOptions } from "@/lib/forms/canonical-options";
...
export function NormalChallengeBuilderStep(props: Props) {
  ...
  if (step === 0) return <><Heading step={step} /><div className="mt-9 grid gap-7">
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      <div data-field="category"><SearchSelect label="Category *" value={form.category} options={CHALLENGE_CATEGORY_OPTIONS} placeholder="Choose category" onChange={(value) => { update("category", value); update("subcategory", ""); }} error={errors.category} /></div>
      <div data-field="subcategory"><SearchSelect label="Subcategory *" value={form.subcategory} options={challengeSubcategoryOptions(form.category)} placeholder={category ? "Choose subcategory" : "Choose a category first"} disabled={!category} onChange={(value) => update("subcategory", value)} error={errors.subcategory} /></div>
    </div>
  ...
  if (step === 4) return <><Heading step={step} /><div className="mt-9 space-y-8">
    <div data-field="timeZone" className="max-w-md"><SearchSelect label="Timezone *" value={form.timeZone} options={TIMEZONE_OPTIONS} onChange={(value) => update("timeZone", value)} error={errors.timeZone} /><p className="mt-2 text-sm text-slate-600">All challenge times are shown in this timezone.</p></div>
