...
export function NormalChallengeBuilderStep(props: Props) {
  ...
  if (step === 1) return <><Heading step={step} /><div className="mt-9 grid gap-8">
    <Section label="How can people join?"><div className="rounded-[8px] border border-black/10 bg-slate-50 p-4"><p className="font-bold text-slate-950">Anyone eligible can join</p><p className="mt-1 text-sm leading-6 text-slate-600">Eligibility, capacity, payment, and schedule rules are checked automatically.</p></div></Section>
...
function Review({ form, readiness, edit }: { form: NormalChallengeForm; readiness: NormalChallengeReadiness; edit: (step: number) => void }) {
  ...
  const cards = [
    ...
    ["Eligibility", `Anyone eligible can join; ${form.locationEligibility === "worldwide" ? "worldwide" : `${form.eligibleCountries.length} selected countries`}; ${form.capacityMode === "unlimited" ? "No fixed capacity." : `Limited to ${form.maxParticipants || "not set"} participants. ${form.waitlistEnabled ? "Waitlist is enabled when the limit is reached." : ""}`}`, 1],
