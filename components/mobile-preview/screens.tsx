  ...
  return (
    <div className="flex min-h-[760px] flex-col items-center justify-between px-1 py-8 text-center">
      ...
      <div className="flex flex-col items-center">
        ...
        <p className="mt-4 max-w-[285px] text-sm leading-6 text-slate-300">Premium challenges, verified submissions, DoroCoin voting, and real winner moments.</p>
      </div>
    ...
    <div className="flex min-h-[760px] flex-col justify-between py-3 text-center">
      <div className="space-y-5">
        ...
        <div>
          ...
          <p className="mx-auto mt-4 max-w-[300px] text-sm leading-6 text-slate-300">Join challenges, submit polished media, vote with DoroCoins, and watch verified winners rise.</p>
        </div>
        <div className="grid gap-3 text-left">
          ...
          <MobileCard className="p-4"><div className="flex items-center gap-3"><Vote className="text-[var(--gold)]" size={20} /><div><h3 className="font-black">Vote with DoroCoin</h3><p className="text-xs text-slate-400">Weighted votes, premium boosts, and transparent rankings.</p></div></div></MobileCard>
        </div>
      </div>
...
function SubmissionScreen({ selectedSubmission, onGo }: ScreenProps) {
  ...
  return <div className="-mx-4 -mt-3 pb-20"><img src={submission.mediaUrl} alt={submission.title} className="h-[430px] w-full object-cover" /><div className="space-y-5 px-4 pt-5"><div><MobileBadge>Rank #{submission.rank}</MobileBadge><h1 className="mt-3 text-3xl font-black">{submission.title}</h1><p className="mt-2 text-sm text-slate-300">by {submission.creator} - {submission.challengeTitle}</p></div><div className="grid grid-cols-3 gap-3"><MiniMetric label="Votes" value={submission.votes.toLocaleString()} /><MiniMetric label="Rank" value={`#${submission.rank}`} /><MiniMetric label="Status" value="Live" /></div><MobileCard className="p-4"><h3 className="font-black text-[var(--gold)]">Submission Preview</h3><p className="mt-2 text-sm leading-6 text-slate-300">Verified voting is active. Free users get one vote per challenge/day. Paid votes use DoroCoins.</p></MobileCard></div><StickyActions><MobileButton onClick={() => onGo("vote")}>Vote for Entry</MobileButton><MobileButton variant="secondary" onClick={() => onGo("challenge-detail")}>Challenge</MobileButton></StickyActions></div>;
}

function VoteScreen({ onSheet }: ScreenProps) {
  const [selected, setSelected] = useState("100");
  return <div className="space-y-5 pb-24"><ScreenTitle title="Vote" subtitle="Spend DoroCoins to support your favorite submission." /><SubmissionRow submission={submissions[0]} /><MobileCard className="p-4"><div className="flex items-center justify-between"><span className="font-bold text-slate-300">Wallet balance</span><span className="text-2xl font-black text-[var(--gold)]">500</span></div></MobileCard><VoteAmountGrid selected={selected} setSelected={setSelected} /><label className="flex gap-3 rounded-[14px] border border-white/10 bg-[#121212] p-4 text-sm font-bold text-slate-200"><input type="checkbox" defaultChecked /> Paid votes are final sale and subject to voting policy.</label><StickyActions><MobileButton onClick={() => onSheet({ title: "Confirm votes", body: `${selected} votes will be recorded for Rainline Reflections. ${selected} DoroCoins will be deducted.`, action: "Votes recorded" })}>Confirm Vote Purchase</MobileButton></StickyActions></div>;
}

function WalletScreen({ onSheet }: ScreenProps) {
  return <div className="space-y-5 pb-4"><ScreenTitle title="Wallet" subtitle="Use DoroCoins for votes, boosts, premium entries, and rewards." /><MobileCard className="border-yellow-400/30 bg-yellow-500/10 p-5"><p className="text-sm font-bold text-slate-300">Available Balance</p><div className="mt-2 text-5xl font-black text-[var(--gold)]">500</div></MobileCard><section><SectionHeader title="Buy DoroCoins" /><div className="space-y-3">{walletPackages.map((pack) => <WalletPackage key={pack.id} pack={pack} onClick={() => onSheet({ title: `${pack.coins} DoroCoins`, body: `${pack.label} package for ${pack.price}. Checkout will connect to Stripe in production.`, action: "Checkout preview started" })} />)}</div></section><section><SectionHeader title="Recent activity" /><MobileListRow title="Vote spend" meta="Neon City Photo Battle" value="-100" danger /><MobileListRow title="Admin grant" meta="Launch reward" value="+500" /></section></div>;
}
