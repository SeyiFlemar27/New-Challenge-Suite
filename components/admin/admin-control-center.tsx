...
const queueActions: Record<string, Array<{ action: string; label: string; dangerous?: boolean }>> = {
  ...
  challenges: [{ action: "approve", label: "Publish" }, { action: "request_changes", label: "Request changes" }, { action: "flag", label: "Flag" }, { action: "reject", label: "Reject", dangerous: true }, { action: "archive", label: "Archive" }, { action: "suspend", label: "Suspend", dangerous: true }, { action: "add_note", label: "Add note" }],
  ...
  "enterprise-applications": [{ action: "approve", label: "Approve" }, { action: "request_info", label: "Request Info" }, { action: "reject", label: "Reject", dangerous: true }, { action: "suspend_access", label: "Suspend Access", dangerous: true }, { action: "revoke_access", label: "Revoke Access", dangerous: true }, { action: "restore_access", label: "Restore Access" }, { action: "add_note", label: "Add Internal Note" }]
};
...
function availableQueueActions(section: string, status: string, accessStatus = "active") {
  return (queueActions[section] ?? []).filter((item) => {
    if (section === "enterprise-applications") {
      if (status === "approved") {
        if (accessStatus === "suspended") return ["restore_access", "revoke_access", "add_note"].includes(item.action);
        if (accessStatus === "revoked") return ["restore_access", "add_note"].includes(item.action);
        return ["suspend_access", "revoke_access", "add_note"].includes(item.action);
      }
      if (status === "rejected") return item.action === "add_note";
...
export function AdminControlCenter({ section = "overview" }: { section?: string }) {
  ...
  const [enterpriseExpiresAt, setEnterpriseExpiresAt] = useState("");
  const [enterpriseReapplyAllowed, setEnterpriseReapplyAllowed] = useState(true);
  const [enterpriseReapplyAfter, setEnterpriseReapplyAfter] = useState("");
  ...
  function openAction(record: AdminRecord, action: string) {
    ...
    setEnterpriseExpiresAt("");
    setEnterpriseReapplyAllowed(true);
    setEnterpriseReapplyAfter("");
  }
  async function confirmAction() {
    ...
    const enterpriseProvisioning = normalizedSection === "enterprise-applications" && pendingAction.action === "approve"
      ? { enterpriseRole, enterpriseScope, enterpriseDepartment, enterpriseExpiresAt: enterpriseExpiresAt || null }
      : {};
    const result = await apiRequest("/api/admin/operations", { method: "PATCH", body: JSON.stringify({ type: typeBySection[normalizedSection], id: pendingAction.record.id, action: pendingAction.action, reason, note, expectedVersion: pendingAction.record.currentRevision ?? pendingAction.record.version, reapplyAllowed: enterpriseReapplyAllowed, reapplyAfter: enterpriseReapplyAfter || null, ...enterpriseProvisioning }) });
    setSubmitting(false);
  ...
  return (
    <>
      ...
      {pendingAction ? <ActionDialog action={pendingAction.action} reason={reason} note={note} setReason={setReason} setNote={setNote} enterpriseApplicationAction={normalizedSection === "enterprise-applications"} enterpriseProvisioning={normalizedSection === "enterprise-applications" && pendingAction.action === "approve"} enterpriseRole={enterpriseRole} enterpriseScope={enterpriseScope} enterpriseDepartment={enterpriseDepartment} enterpriseExpiresAt={enterpriseExpiresAt} enterpriseReapplyAllowed={enterpriseReapplyAllowed} enterpriseReapplyAfter={enterpriseReapplyAfter} setEnterpriseRole={setEnterpriseRole} setEnterpriseScope={setEnterpriseScope} setEnterpriseDepartment={setEnterpriseDepartment} setEnterpriseExpiresAt={setEnterpriseExpiresAt} setEnterpriseReapplyAllowed={setEnterpriseReapplyAllowed} setEnterpriseReapplyAfter={setEnterpriseReapplyAfter} submitting={submitting} onCancel={() => setPendingAction(null)} onConfirm={confirmAction} /> : null}
    </>
  );
}
...
function BaseActionDialog({ action, reason, note, setReason, setNote, enterpriseApplicationAction, enterpriseProvisioning, enterpriseRole, enterpriseScope, enterpriseDepartment, enterpriseExpiresAt, enterpriseReapplyAllowed, enterpriseReapplyAfter, setEnterpriseRole, setEnterpriseScope, setEnterpriseDepartment, setEnterpriseExpiresAt, setEnterpriseReapplyAllowed, setEnterpriseReapplyAfter, submitting, onCancel, onConfirm }: { action: string; reason: string; note: string; setReason: (value: string) => void; setNote: (value: string) => void; enterpriseApplicationAction: boolean; enterpriseProvisioning: boolean; enterpriseRole: string; enterpriseScope: string; enterpriseDepartment: string; enterpriseExpiresAt: string; enterpriseReapplyAllowed: boolean; enterpriseReapplyAfter: string; setEnterpriseRole: (value: string) => void; setEnterpriseScope: (value: string) => void; setEnterpriseDepartment: (value: string) => void; setEnterpriseExpiresAt: (value: string) => void; setEnterpriseReapplyAllowed: (value: boolean) => void; setEnterpriseReapplyAfter: (value: string) => void; submitting: boolean; onCancel: () => void; onConfirm: () => void }) {
  const needsReason = reasonRequired.has(action);
  const noteOnly = action === "add_note";
  return <div className="fixed inset-0 z-[100] flex items-center justify-center p-5" role="dialog" aria-modal="true" aria-label="Confirm admin action"><button className="absolute inset-0 bg-black/85" onClick={onCancel} aria-label="Cancel admin action" /><Card className="relative z-10 max-h-[90vh] w-full max-w-xl overflow-y-auto border-[var(--gold)]/25 p-6 sm:p-8"><p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--gold)]">Review decision</p><h2 className="mt-3 text-2xl font-black">{friendlyLabel(action)}</h2><div className="mt-4 rounded-[8px] border border-white/10 p-4"><h3 className="font-black">What happens next</h3><p className="mt-2 text-sm leading-6 text-slate-300">The server will verify your permission and the record's current state, save the decision, and add an audit event. This control does not execute an external payment, payout, or refund.</p></div>{enterpriseProvisioning ? <div className="mt-6 grid gap-4 sm:grid-cols-2"><label><span className="mb-2 block text-sm font-bold">Enterprise role</span><select className={inputClass} value={enterpriseRole} onChange={(event) => setEnterpriseRole(event.target.value)}><option value="platform_owner">Platform Owner</option><option value="operations">Operations</option><option value="reviewer">Reviewer</option><option value="finance">Finance</option><option value="partnerships">Partnerships</option><option value="support_safety">Support &amp; Safety</option></select></label><label><span className="mb-2 block text-sm font-bold">Access scope</span><select className={inputClass} value={enterpriseScope} onChange={(event) => setEnterpriseScope(event.target.value)}><option value="assigned_only">Assigned Only</option><option value="all_official">All Official</option><option value="read_all_edit_assigned">Read All / Edit Assigned</option></select></label><label className="sm:col-span-2"><span className="mb-2 block text-sm font-bold">Department</span><input className={inputClass} value={enterpriseDepartment} maxLength={100} onChange={(event) => setEnterpriseDepartment(event.target.value)} /></label><label className="sm:col-span-2"><span className="mb-2 block text-sm font-bold">Optional access expiration</span><input className={inputClass} type="datetime-local" value={enterpriseExpiresAt} onChange={(event) => setEnterpriseExpiresAt(event.target.value)} /><span className="mt-1 block text-xs text-slate-400">Leave blank for access with no expiration.</span></label><p className="sm:col-span-2 text-xs leading-5 text-slate-400">Permissions are derived from the selected role. Access scope is enforced again by Enterprise APIs.</p></div> : null}{needsReason ? <label className="mt-6 block"><span className="mb-2 block text-sm font-bold">Reason required</span><textarea className={textareaClass} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Explain why this action is required." /></label> : null}<label className="mt-5 block"><span className="mb-2 block text-sm font-bold">{noteOnly ? "Internal note required" : "Internal note (optional)"}</span><textarea className={textareaClass} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Visible only to authorized administrators." /></label><div className="mt-6 grid gap-3 sm:grid-cols-2"><Button variant="secondary" onClick={onCancel}>Cancel</Button><Button onClick={onConfirm} disabled={submitting}>{submitting ? "Saving..." : `Confirm ${friendlyLabel(action)}`}</Button></div></Card></div>;
}

type ActionDialogProps = React.ComponentProps<typeof BaseActionDialog>;

function ActionDialog(props: ActionDialogProps) {
  if (!props.enterpriseApplicationAction || props.action !== "reject") return <BaseActionDialog {...props} />;
  return <div className="fixed inset-0 z-[100] flex items-center justify-center p-5" role="dialog" aria-modal="true" aria-label="Reject Enterprise application"><button className="absolute inset-0 bg-black/85" onClick={props.onCancel} aria-label="Cancel admin action" /><Card className="relative z-10 max-h-[90vh] w-full max-w-xl overflow-y-auto border-[var(--gold)]/25 p-6 sm:p-8"><p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--gold)]">Review decision</p><h2 className="mt-3 text-2xl font-black">Reject Enterprise Application</h2><label className="mt-6 block"><span className="mb-2 block text-sm font-bold">Reason required</span><textarea className={textareaClass} value={props.reason} onChange={(event) => props.setReason(event.target.value)} placeholder="Explain the decision to the applicant." /></label><div className="mt-5 rounded-[8px] border border-white/10 p-4"><label className="flex min-h-11 items-center gap-3 font-bold"><input type="checkbox" checked={props.enterpriseReapplyAllowed} onChange={(event) => props.setEnterpriseReapplyAllowed(event.target.checked)} /><span>Allow this applicant to reapply</span></label>{props.enterpriseReapplyAllowed ? <label className="mt-4 block"><span className="mb-2 block text-sm font-bold">Optional reapplication date</span><input className={inputClass} type="datetime-local" value={props.enterpriseReapplyAfter} onChange={(event) => props.setEnterpriseReapplyAfter(event.target.value)} /><span className="mt-1 block text-xs text-slate-400">Leave blank to allow immediate editing and resubmission.</span></label> : null}</div><label className="mt-5 block"><span className="mb-2 block text-sm font-bold">Internal note (optional)</span><textarea className={textareaClass} value={props.note} onChange={(event) => props.setNote(event.target.value)} placeholder="Visible only to authorized administrators." /></label><div className="mt-6 grid gap-3 sm:grid-cols-2"><Button variant="secondary" onClick={props.onCancel}>Cancel</Button><Button onClick={props.onConfirm} disabled={props.submitting}>{props.submitting ? "Saving..." : "Confirm Reject"}</Button></div></Card></div>;
}
