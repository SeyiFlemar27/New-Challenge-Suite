...
export function EnterpriseSectionPage({ kind }: { kind: Kind }) {
  ...
  return <AppShell><div className="mx-auto max-w-7xl"><PageTitle title={config.title} subtitle={config.subtitle} icon={<Icon className="text-[var(--gold)]" />} />{error ? <Card className="mt-7 p-6 text-red-700">{error}<div><LinkButton href="/contact" className="mt-5">Contact Support</LinkButton></div></Card> : !data ? <Card className="mt-7 h-72 animate-pulse" /> : <>
    ...
    {!(["analytics", "team"] as Kind[]).includes(kind) ? <><div className="mt-7 max-w-md"><label className="relative block"><Search className="absolute left-3 top-3.5 text-slate-400" size={18} /><span className="sr-only">Search official challenges</span><input className={`${inputClass} pl-10`} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search official challenges" /></label></div><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <Card key={item.id} className="p-5"><p className="text-xs font-black uppercase text-amber-700">{item.displayStatus}</p><h2 className="mt-2 text-lg font-black text-slate-950">{item.title}</h2><p className="mt-1 text-sm text-slate-600">{item.assignment ?? item.category}</p><LinkButton href={`/challenges/${item.id}/manage`} className="mt-5">Open Operations</LinkButton></Card>)}{!items.length ? <Card className="p-6 text-sm text-slate-600">{kind === "assigned" ? "No work is assigned to you right now." : "No records in this Enterprise scope."}</Card> : null}</div></> : null}
  </>}</div></AppShell>;
}
