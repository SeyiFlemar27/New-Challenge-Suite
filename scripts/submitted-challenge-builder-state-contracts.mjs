...
for (const copy of ["Challenge Submitted", "Your challenge has been sent for review.", "Status: Under Review.", "Editing is unavailable until an admin requests changes.", "View Challenge", "Back to Dashboard", "Contact Support"]) assert.ok(submittedState.includes(copy), `missing submitted-state copy: ${copy}`);
...
const editableWizardRender = builder.indexOf('return (\n    <AppShell>');
