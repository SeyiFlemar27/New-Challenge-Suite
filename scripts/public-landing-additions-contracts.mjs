...
export async function run(name) {
  ...
  const checks = {
    ...
    "public-ambassador-video-scroll-five-total-placeholders": () => assert.equal((config.match(/status: "placeholder" \}/g) ?? []).length, 5),
    ...
    "public-ambassador-video-scroll-vertical-cards": () => assert.match(home, /aspect-\[16\/10\]/),
    "public-ambassador-video-scroll-reference-layout": () => { assert.match(home, /aspect-\[16\/10\]/); assert.match(home, /<h3[^>]*>\{item\.title\}<\/h3>/); assert.match(home, /<p[^>]*>\{item\.subtitle\}<\/p>/); },
    "public-ambassador-video-scroll-three-visible-desktop": () => assert.match(home, /lg:basis-\[calc\(\(100%-2rem\)\/3\)\]/),
    "public-ambassador-video-scroll-horizontal-overflow-track": () => { assert.match(home, /overflow-x-auto/); assert.match(home, /overscroll-x-contain/); },
    "public-ambassador-video-scroll-mobile-one-card": () => assert.match(home, /basis-\[86%\]/),
    "public-ambassador-video-scroll-tablet-two-cards": () => assert.match(home, /sm:basis-\[calc\(\(100%-1rem\)\/2\)\]/),
    "public-ambassador-video-scroll-scroll-snap": () => { assert.match(home, /snap-x snap-mandatory/); assert.match(home, /snap-start/); },
    "public-ambassador-video-scroll-active-card-state": () => { assert.match(home, /data-active=\{featured\|\|undefined\}/); assert.match(home, /border-\[#c5aa00\]/); },
    "public-ambassador-video-placeholder-no-broken-modal": () => { assert.match(home, /const available=item\.status==="active"&&Boolean\(item\.videoUrl\)/); assert.match(home, /available\?<button/); assert.doesNotMatch(config, /videoUrl: "https?:/); },
    ...
    "public-ambassador-app-cta-coming-soon-safe": () => { assert.match(home, /Get the app when it launches/); assert.match(home, /label\} - Coming Soon/); assert.match(config, /PUBLIC_APP_STORE_URL: string \| null = null/); assert.match(config, /PUBLIC_GOOGLE_PLAY_URL: string \| null = null/); },
    "public-ambassador-video-scroll-mobile-no-overflow": () => { assert.match(home, /basis-\[86%\]/); assert.match(home, /overflow-x-auto/); assert.match(home, /overscroll-x-contain/); },
