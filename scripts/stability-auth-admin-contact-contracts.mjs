...
assert.match(deletion, /retainedHistoryQueries/);
assert.match(deletion, /profileVisibility: "private"/);
assert.match(deletion, /accountStatus: "deactivated"/);
assert.match(deletion, /adminAuth\.deleteUser/);
assert.match(deletion, /adminAuth\.updateUser\(user!\.uid, \{ disabled: true \}\)/);
for (const retained of ["cashLedger", "challengeEntryPayments", "paidVotePurchases", "sponsorContributions", "predictionPayments", "doroCoinPurchases", "withdrawalRequests", "kycRecords", "challengeSettlements", "auditLogs", "stripeCustomerId"]) assert.match(deletion, new RegExp(retained));
