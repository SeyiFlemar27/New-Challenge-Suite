...
assert(builder.indexOf("persist(6)") < builder.indexOf("publishChallengeDraft(id,payload)"), "Submit for Review must persist latest changes first");
...
for (const title of ["Basics", "Participation", "Entry & Submission", "Competition", "Rewards", "Schedule", "Review"]) assert((builder + read("lib/challenge-builder-foundation.ts")).includes(`"${title}"`), `missing Normal Challenge step ${title}`);
...
assert(readiness.includes("VOTING_BEFORE_SUBMISSION_CLOSE") && readiness.includes("WINNER_SPLIT_INVALID") && readiness.includes("COVER_VIDEO_REQUIRED"), "cross-step readiness checks are incomplete");
...
assert(media.indexOf("videoUrl ? <div") < media.indexOf("imageUrls.length ? <div"), "public challenge media must remain video-first");
