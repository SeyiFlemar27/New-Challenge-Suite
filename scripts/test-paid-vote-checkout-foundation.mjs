...
assert(votesPage.includes("Paid votes setup required"), "paid vote UI must remain setup-safe when payment activation is not ready.");
assert(votesPage.includes("granted only after Stripe webhook confirmation"), "paid vote UI must explain webhook-only grant.");
...
assert(helper.includes("creditConsumptionActive: false"), "paid vote consumption gap must stay explicit.");
...
assert(voteRoute.includes("voteMode: body.voteMode"), "existing free/DoroCoin voting route must remain separate.");
