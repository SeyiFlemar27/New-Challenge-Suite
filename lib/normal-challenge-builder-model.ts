...
export type NormalChallengeForm = {
  ...
  participationMode: "open"; locationEligibility: "worldwide" | "selected"; eligibleCountries: string[]; ageRestrictionMode: "none" | "minimum"; minimumAge: string; capacityMode: "unlimited" | "limited"; maxParticipants: string; waitlistEnabled: boolean; hideParticipantList: boolean;
  paidEntryEnabled: boolean; entryFeeAmount: string; paidVotesEnabled: boolean; sponsorReady: boolean; sponsorshipGoal: string; preferredSponsorCategory: string; sponsorNote: string; numberOfWinners: number; winnerPrizeAmounts: string[];
...
export function freshNormalChallengeForm(now = Date.now()): NormalChallengeForm {
  ...
  return {
    ...
    paidEntryEnabled: false, entryFeeAmount: "", paidVotesEnabled: false, sponsorReady: false, sponsorshipGoal: "increase_prize_pool", preferredSponsorCategory: "", sponsorNote: "", numberOfWinners: 1, winnerPrizeAmounts: [""],
    images: [], video: null,
...
export function normalChallengePayload(form: NormalChallengeForm, challengeId = "") {
  ...
  const maxParticipants = form.capacityMode === "unlimited" ? null : normalizeNormalChallengeCapacity(form.maxParticipants, "limited");
  ...
  return {
    ...
    participationMode: "open", requiresParticipantApproval: false, participantApprovalMode: "automatic", locationEligibility: form.locationEligibility, eligibleCountries: form.locationEligibility === "selected" ? form.eligibleCountries : [], eligibleCountry: "", ageRestrictionMode: form.ageRestrictionMode, minimumAge: form.ageRestrictionMode === "minimum" ? Number(form.minimumAge || 0) : 0, capacityMode: form.capacityMode, maxParticipants, waitlistEnabled: form.capacityMode === "limited" && form.waitlistEnabled, hideParticipantList: form.hideParticipantList, teamParticipationEnabled: false,
    ...
    monetization: { enabled: form.paidEntryEnabled, paidEntryRequested: form.paidEntryEnabled, entryFeeAmountCents: Math.round(Number(form.entryFeeAmount || 0) * 100), currency: "USD", sponsorReady: form.sponsorReady, prizePoolRequested: true, paidVotesRequested: form.paidVotesEnabled, status: form.paidEntryEnabled || form.sponsorReady ? "setup_required" : "not_requested", paymentActive: false, checkoutActive: false, ledgerCreationEnabled: false, prizeReleaseActive: false, payoutReleaseActive: false, sponsorshipGoal: form.sponsorshipGoal, preferredSponsorCategory: form.preferredSponsorCategory.trim(), sponsorNote: form.sponsorNote.trim(), placements: [] },
