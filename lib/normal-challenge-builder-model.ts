...
export type NormalChallengeForm = {
  ...
  paidEntryEnabled: boolean; entryFeeAmount: string; paidVotesEnabled: boolean; sponsorReady: boolean; sponsorshipGoal: string; preferredSponsorCategories: string[]; sponsorNote: string; numberOfWinners: number; winnerPrizeAmounts: string[];
...
export function freshNormalChallengeForm(now = Date.now()): NormalChallengeForm {
  ...
  return {
    ...
    paidEntryEnabled: false, entryFeeAmount: "", paidVotesEnabled: false, sponsorReady: false, sponsorshipGoal: "increase_prize_pool", preferredSponsorCategories: [], sponsorNote: "", numberOfWinners: 1, winnerPrizeAmounts: [""],
    images: [], video: null,
...
export function normalChallengePayload(form: NormalChallengeForm, challengeId = "") {
  ...
  return {
    ...
    monetization: { enabled: form.paidEntryEnabled, paidEntryRequested: form.paidEntryEnabled, entryFeeAmountCents: Math.round(Number(form.entryFeeAmount || 0) * 100), currency: "USD", sponsorReady: form.sponsorReady, prizePoolRequested: true, paidVotesRequested: form.paidVotesEnabled, status: form.paidEntryEnabled || form.sponsorReady ? "setup_required" : "not_requested", paymentActive: false, checkoutActive: false, ledgerCreationEnabled: false, prizeReleaseActive: false, payoutReleaseActive: false, sponsorshipGoal: form.sponsorshipGoal, preferredSponsorCategories: normalizeSponsorCategories(form.preferredSponsorCategories), preferredSponsorCategory: normalizeSponsorCategories(form.preferredSponsorCategories)[0] ?? "", sponsorNote: form.sponsorNote.trim(), placements: [] },
