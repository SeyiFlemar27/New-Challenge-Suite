...
export const serverChallengeCreateSchema = z.object({
  ...
  officialChallenge: z.coerce.boolean().default(false),
  ownershipType: z.enum(["creator_personal", "enterprise_personal", "challenge_suite_official"]).default("creator_personal"),
  ...
  if (normalChallenge) {
    if (value.builderVersion === "normal_v2" && (value.participationMode !== "open" || value.requiresParticipantApproval || value.participantApprovalMode !== "automatic")) {
      ctx.addIssue({ code: "custom", path: ["participationMode"], message: "Normal Challenges use automatic eligible joining." });
    }
  ...
  const maxParticipants = capacityMode === "unlimited" ? null : value.maxParticipants;
  return { ...value, capacityMode, maxParticipants, eventCapacity: value.isLiveEvent ? maxParticipants : value.eventCapacity };
});
